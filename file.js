...
async function moveFiles(srcDir, destDir, regExp = false, mode = copy) {
  if (!fs.existsSync(srcDir)) throw new Error(`⛔ dir ${srcDir} does not exist.`);
  if (srcDir === destDir) throw new Error(`⛔ Destination is pointing to the source directory.`);
  //create destDir
  fs.mkdir(destDir, { recursive: true }, err => {
    if (err) throw new Error(`⛔ Failed to create dir ${destDir}\r\n${err}`);

    fs.readdir(srcDir, { withFileTypes: true, recursive: true }, (err, files) => {
      if (err) throw new Error(`⛔ Failed to read files from ${destDir}\r\n${err}`);
      ...
      files.forEach(file => {
        ...
        fs.copyFile(srcFilePath, destFilePath, err => {
          if (err) throw new Error(`⛔ Failed to copy file \r\nFROM:${srcFilePath}\r\nTO:${destFilePath}\r\n${err}`);
